...
%%
Xp = [particle.xf, particle.yf, particle.zf];
Vp = [particle.vx, particle.vy, particle.vz];

R = rotationMatrix(Vp, [1,0,0]);

rXp = (R*Xp')';

for i = 1:numel(tracer.xf)
    Xf = [tracer.xf(i), tracer.yf(i), tracer.zf(i)];
    rXf = (R*Xf')';
    
    [rho, theta, z] = cartToCylind(rXp, rXf);
end
