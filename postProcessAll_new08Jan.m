...
fpathP_slipVeloData = ['.' filesep 'slipVeloData1000'];
...
for kexp = 1:Nexp
    ...
    load([fpathT_tracks fname],'tracklong_tracers')
    load([fpathP_tracks fname],'tracklong_particle')
    % find terminal state where a ~= 0
    [maxTimeLength,startTime,endTime,ThresErrorTS] = findTerminalState(tracklong_particle,1000);    
    
    % cylindrical coords + bin
...
nbinr = 50;
nbint = 50;
meanUrel.x = zeros(nbinr,nbint)*NaN;
meanUrel.y = zeros(nbinr,nbint)*NaN;
meanUrel.z = zeros(nbinr,nbint)*NaN;
meanUrel.norm = zeros(nbinr,nbint)*NaN;
count = zeros(nbinr,nbint)*NaN;

fieldname = fieldnames(meanUrel);
...
%     U = vertcat(slipVeloCylind.Urelnorm);
    
    rho2D = sqrt(rho.^2 + z.^2);
    theta2D = atan2(rho,z);

    % Iterate over each bin and calculate the mean value of U
    for i = 1:nbinr-1
        for j = 1:nbint-1
            ...
            if numel(ind)~=0
                if isnan(count(j,i))
                    ...
                    for kf = 1:numel(fieldname)
                        meanUrel.(fieldname{kf})(j,i) = 0;
                    end
                end
                
                count(j,i) = count(j,i)+numel(ind);
                for kf = 1:numel(fieldname)
                    meanUrel.(fieldname{kf})(j,i) = meanUrel.(fieldname{kf})(j,i) + sum(Urel.(fieldname{kf})(ind));
                end
            end
        end
    end
...
for kf = 1:numel(fieldname)
    meanUrel.(fieldname{kf}) = meanUrel.(fieldname{kf})./count;
end
...
for kf = 1:numel(fieldname)
    ...
    title(fieldname{kf})
    savefig_custom(pwd,['slipVelo_' fieldname{kf}],8,7)
end
