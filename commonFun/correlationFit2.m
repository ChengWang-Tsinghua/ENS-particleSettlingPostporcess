function Rfit = correlationFit2(Rtau,Rmean,xmin,xmax,type,if2layers,ifbounded)
...
if strcmp(type,'V') == 1   
    y=Rmean/Rmean(1);
%     y=1-S2Lx.mean./(2*Rmean(1)); 
else
    y=Rmean/Rmean(1);
end
%% fit range
     startpoints=[max(y) 0.1 0.01]; lowerpoints=[0.8*max(y) 0 0]; upperpoints=[1.20*max(y) 0.2 0.2];
...
%% convert to structure array
    Rfit.x=Rtau;
    Rfit.y=Rmean/Rmean(1); 
    % Rfit.y=1-S2Lx.mean./(2*Rmean(1)); 
